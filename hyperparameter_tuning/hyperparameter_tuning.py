...
def objective(trial):
    ...
    hidden_dim = [
        trial.suggest_int("hidden_dim_1", 96, 128, step=32),
        trial.suggest_int("hidden_dim_2", 512, 2048, step=256),
        trial.suggest_int("hidden_dim_3", 320, 512, step=64)
    ]
    num_heads = [
        trial.suggest_categorical("num_heads_1", [2]),
        trial.suggest_categorical("num_heads_2", [64]),
        trial.suggest_categorical("num_heads_3", [2])
    ]
    num_layers = [
        trial.suggest_int("num_layers_1", 1, 3),
        trial.suggest_int("num_layers_2", 2, 6)
    ]
    learning_rate = trial.suggest_float("learning_rate", 3e-4, 3e-4)
    ...
    _, _, _, _, best_val_acc = main(hidden_dim, num_heads, num_layers, learning_rate,
                                    ...
                                    plot=False, head_layer_sizes=head_layer_sizes)
...
study_name = "tuning_MultiModalityFusion_07"  # Unique identifier of the study.
